...
target_table <- Sys.getenv("TARGET_TABLE", unset = 'covid_data.regional_aggregation_alt_5')
...
all_dates <- copy_to(conn, tibble(date = seq.Date(from = date_start, to = date_end, by = 1)),
                     'regional_aggregations_alt_5_tmp', overwrite = TRUE)
...
# Compute table corresponding to the slice_date
{
  ...
  linelist <- get_table(conn, 'prod.covid_19_patientlinelist', slice_date = slice_time) %>%
    ...
    transmute(cprnr = cpr,
              ...
              prdate = sampledate,
              newlyadmdate,
              end_date = pmin(firstdsc, as.Date(newlyadmdate + days(90)), na.rm = T),
              covid = firstadmclassv2 %like% '% pga. covid-19') %>%
    left_join(miba, by = c("cprnr", "prdate")) %>%
    compute()
  # Hosp end : firstdsc ELLER newlyadmdate + 90 days (whichever is first)
...
#
# get_table(conn, 'covid_19_patientlinelist') %>%
#   filter(firstadmclassv2 %like% '%pga. covid-19', is.na(epilprfirstadm)) %>%
#   head(1000) %>%
#   collect() %>%
#   View()
#
# tmp1 <- get_table(conn, 'covid_19_patientlinelist') %>%
#   filter(firstadmclassv2 %like% '%pga. covid-19') %>%
#   select(newlyadm, newlyadmdate, epilprfirst_in, epilprfirst_out, epilprfirstadm, firstadm, firstdsc)
#
# tmp1 %>% count(newlyadmdate == firstadm, firstdsc == epilprfirst_out, newlyadm) %>% arrange(newlyadm)


# Hosp start : newlyadmdate
# Hosp end : firstdsc ELLER newlyadmdate + 60 (whichever is first)
