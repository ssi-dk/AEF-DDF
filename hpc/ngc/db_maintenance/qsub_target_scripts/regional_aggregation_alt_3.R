...
suppressPackageStartupMessages({
  library(mg, lib.loc = ifelse(packageVersion("mg") < '0.1.5', '/ngc/projects/ssi_mg/common/R_packages/', '/ngc/projects/ssi_mg/rassky/R_packages/local_builds/'))
  library(purrr)
})

target_table <- Sys.getenv("TARGET_TABLE", unset = 'covid_data.regional_aggregation_alt_3')
...
all_dates <- copy_to(conn, tibble(date = seq.Date(from = date_start, to = date_end, by = 1)),
                     'regional_aggregations_alt_3_tmp', overwrite = TRUE)
...
# Compute table corresponding to the slice_date
{
  ...
  ## linelist ##
  linelist <- get_table(conn, 'prod.covid_19_patientlinelist', slice_date = slice_time) %>%
    filter(!is.na(newlyadmdate)) %>%
    transmute(cprnr = cpr,
              episodekey,
              prdate = sampledate,
              newlyadmdate,
              end_date = pmin(firstdsc, lastdsc, as.Date(newlyadmdate + days(90)), na.rm = T),
              covid = firstadmclassv2 %like% '% pga. covid-19') %>%
    left_join(miba, by = c("cprnr", "prdate")) %>%
    compute()
  # Hosp end : firstdsc, lastdsc, or newlyadmdate + 90 days (whichever is first)
  ...
  hosp <- linelist %>%
    inner_join(all_dates, sql_on = '("RHS"."date" >= DATE("LHS"."newlyadmdate") AND ("RHS"."date" < DATE("LHS"."end_date") OR "LHS"."end_date" IS NULL))')
...
#
# slice_time <- '2022-10-01 09:00:00'
# date_start <- as.Date("2020-03-01")
# date_end <- as.Date(slice_time) - days(2)
#
# all_dates <- copy_to(conn, tibble(date = seq.Date(from = date_start, to = date_end, by = 1)),
#                      'regional_aggregations_tmp2', overwrite = TRUE)
#
#
# t <- get_table(conn, 'prod.covid_19_patientlinelist', slice_date = slice_time) %>%
#   filter(!is.na(newlyadmdate)) %>%
#   transmute(cprnr = cpr,
#             episodekey,
#             prdate = sampledate,
#             newlyadmdate,
#             end_date = pmin(lastdsc, as.Date(newlyadmdate + days(90)), na.rm = T),
#             covid = firstadmclassv2 %like% '% pga. covid-19') %>%
#   inner_join(all_dates %>% filter(date == '2022-06-14'), sql_on = '("RHS"."date" >= DATE("LHS"."newlyadmdate") AND ("RHS"."date" < DATE("LHS"."end_date") OR "LHS"."end_date" IS NULL))')
#
#
# tmp <- full_join(t %>% mutate(hosp = 1), get_table(conn, 'prod.covid_19_patientlinelist', slice_date = '2022-06-14 09:00:00') %>%
#                    filter(regadmission == 'True') %>% select(cprnr = cpr, episodekey, sampledate, newlyadmdate) %>% mutate(linelist = 1), by = 'cprnr') %>%
#   compute()
# tmp %>% summarise(in_linelist = sum(ifelse(!is.na(linelist), 1, 0)),
#                   in_hosp     = sum(ifelse(!is.na(hosp), 1, 0)),
#                   in_both     = sum(ifelse(!is.na(linelist) && !is.na(hosp), 1, 0)),
#                   in_linelist_but_not_hosp = sum(ifelse(!is.na(linelist) &&  is.na(hosp), 1, 0)),
#                   in_hosp_but_not_linelist = sum(ifelse( is.na(linelist) && !is.na(hosp), 1, 0)))
#
#
# # Some 64 persons are in linelist, but not captured by the above:
# tmp %>% filter(!is.na(linelist), is.na(hosp)) %>%
#   select(cprnr, newlyadmdate.y)
#
# get_table(conn, 'prod.covid_19_patientlinelist', slice_date = slice_time) %>%
#   filter(cpr == '67ba87b10326445978005dc4a9201729') %>%
#   select(cpr, firstdsc, lastdsc, newlyadmdate, regadmission)
#
# get_table(conn, 'prod.covid_19_patientlinelist', slice_date = '2022-06-14 09:00:00') %>%
#   filter(cpr == '67ba87b10326445978005dc4a9201729') %>%
#   select(cpr, firstdsc, lastdsc, newlyadmdate, regadmission)
#
#
#
# get_table(conn, 'prod.covid_19_patientlinelist', slice_date = slice_time) %>%
#   filter(cpr == '201ad35190358b7ae1f209172a2e0a8e') %>%
#   select(cpr, firstdsc, lastdsc, newlyadmdate, regadmission)
#
# get_table(conn, 'prod.covid_19_patientlinelist', slice_date = '2022-06-14 09:00:00') %>%
#   filter(cpr == '201ad35190358b7ae1f209172a2e0a8e') %>%
#   select(cpr, firstdsc, lastdsc, newlyadmdate, regadmission)
#
# # Everyone drops out from snapshot data.
# # .. we should count them for one additional day?
